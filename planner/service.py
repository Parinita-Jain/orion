...
from .llm import get_structured_llm as default_get_structured_llm
from .repair import repair_plan as default_repair_plan
...
class PlanningService:
    """
    Reusable Orion planning service.

    The service contains the planning logic previously located in
    planner_node(). Dependencies used for LLM planning and plan repair
    can be supplied by the caller for testing or specialization.
    """

    def __init__(
        self,
        get_structured_llm=None,
        repair_plan=None,
    ):
        self.get_structured_llm = (
            get_structured_llm
            if get_structured_llm is not None
            else default_get_structured_llm
        )
    ...
    def plan(self, request: str) -> list[RuntimePlanStep]:
        ...
        logger.debug("Planning request: %s", question)
        ...
        if question_lower in GREETINGS:
            ...
            logger.info(
                "Planner selected 'direct' tool using greeting rule"
            )
        ...
        if CALCULATOR_PATTERN.fullmatch(question):
            ...
            logger.info(
                "Planner selected 'calculator' tool using calculator rule"
            )
        ...
        5. If a step depends on the output of a previous step,
        reference the required output field.
        ...
        7. If a step should execute only when a previous step satisfies
        a condition, return a condition.
        ...
        for attempt in range(MAX_REPAIR_ATTEMPTS):
            ...
            repaired = self.repair_plan(
                question,
                result.steps,
                errors,
            )

            repair_errors = validate_plan(
                repaired.steps
            )
            ...
            if repair_errors:
                ...
                logger.warning(
                    "Planner repair produced an invalid plan"
                )
                ...
                logger.info(
                    "Planner repaired execution plan"
                )
